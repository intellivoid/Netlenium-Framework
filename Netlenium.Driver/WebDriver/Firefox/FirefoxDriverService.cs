...
namespace Netlenium.Driver.WebDriver.Firefox
{
    ...
    public sealed class FirefoxDriverService : DriverService
    {
        ...
        {
            get { return browserBinaryPath; }
            set { browserBinaryPath = value; }
        }
        ...
        {
            get { return browserCommunicationPort; }
            set { browserCommunicationPort = value; }
        }
        ...
        {
            get { return connectToRunningBrowser; }
            set { connectToRunningBrowser = value; }
        }
        ...
        {
            get { return openBrowserToolbox; }
            set { openBrowserToolbox = value; }
        }
        ...
        protected override string CommandLineArguments
        {
            get
            {
                ...
                if (connectToRunningBrowser)
                {
                    argsBuilder.Append(" --connect-existing");
                }

                if (browserCommunicationPort > 0)
                {
                    argsBuilder.AppendFormat(CultureInfo.InvariantCulture, " --marionette-port {0}", browserCommunicationPort);
                }

                if (Port > 0)
                {
                    argsBuilder.AppendFormat(CultureInfo.InvariantCulture, " --port {0}", Port);
                }

                if (!string.IsNullOrEmpty(browserBinaryPath))
                {
                    argsBuilder.AppendFormat(CultureInfo.InvariantCulture, " --binary \"{0}\"", browserBinaryPath);
                }

                if (!string.IsNullOrEmpty(host))
                {
                    argsBuilder.AppendFormat(CultureInfo.InvariantCulture, " --host \"{0}\"", host);
                }

                if (loggingLevel != FirefoxDriverLogLevel.Default)
                {
                    argsBuilder.Append(string.Format(CultureInfo.InvariantCulture, " --log {0}", loggingLevel.ToString().ToLowerInvariant()));
                }

                if (openBrowserToolbox)
                {
        ...
        {
            var serviceDirectory = FindDriverServiceExecutable(FirefoxDriverServiceFileName(), FirefoxDriverDownloadUrl);
        ...
        {
            var fileName = DefaultFirefoxDriverServiceFileName;
